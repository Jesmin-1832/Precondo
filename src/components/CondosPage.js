...
//     const mapRef = useRef();
//     const [mapCenter, setMapCenter] = useState([43.65107, -79.347015]); // Default center position
...
//     useEffect(() => {
//         const fetchCondos = async (status = 'selling-now') => {
//             setLoading(true);
//             try {
//                 const response = await fetch(
//                     `https://precondo.ca/wp-json/wp/v2/lv_listing?per_page=12&page=${page}&lv_location=${slugParams['slug']}${status === 'all' ? '' : `&lv_status=${status}`}`
//                 );
...
//                     locality: item.custom_fields?.lv_listing_locality || "Unknown",
...
//         fetchCondos(statusFilter); // Fetch condos when status or page changes
//     }, [statusFilter, page]);
...
//     useEffect(() => {
//         if (!defaultLocation.length) return; // Ensure defaultLocation is loaded

//         let selectedLocation = defaultLocation[0]; // Default to the first location

//         if (slugParams['slug']) {
//             selectedLocation = defaultLocation.find(location =>
//                 location.slug === slugParams['slug'] ||
//                 location.children?.some(child => child.slug === slugParams['slug'])
//             );
//         }
...
//             setSelectedLocation(selectedLocation.slug); // Ensure the selected location remains selected after reload
//         } else {
//             console.warn(`Location not found for slug: ${slugParams['slug']}`);
//         }
//     }, [slugParams, defaultLocation]); // Added defaultLocation as a dependency

//     const updateMapPosition = (latitude, longitude) => {
...
//         if (selectedLocation) {
//             setFilterOption(selectedLocation.id.toString());
//             window.location.href = `/${selectedLocation.slug}`; // Redirect to child location's page if applicable
//         } else {
...
//                                         {location.children?.length > 0 ? ` ${location.name} ` : location.name} ({location.count})
//                                     </option>
